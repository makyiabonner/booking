...
export default function Viewhotel({ selectedHotel, preset }) {
  ...
  const handleSlide = (selectedIndex) =>
    setCurrentSlide((prevNum) => selectedIndex);
  const handleClose = () => setIsActive(false);
  const handleShow = () => setIsActive(true);

  const blockID =
    hotel && hotel.block && hotel.block[0] && hotel.block[0].room_id
      ? hotel.block[0].room_id
      : null;
  const price = hotel
  ...
  const photoGallery =
    hotel && hotel.rooms && blockID ? hotel.rooms[blockID].photos : [];
      ...
      return (
        <Carousel.Item key={index}>
          <img
            ...
            style={{
              backgroundImage: `${
                hotel && photoGallery.length > 0
                  ? `url(${photo.url_original})`
                  : `linear-gradient(45deg, #FFC700 1%, rgb(241, 145, 0) 10%)`
              }`,
      ...
      <section className={styles.hotel_model}>
        {checkSelectHotel() ? (
          <>
            ...
            <div
              className={blockID ? `${styles.hotel_details}` : `${styles.hide}`}
            >
              <h1
                className={blockID ? `${styles.hotel_name}` : `${styles.hide}`}
              >
                {hotel ? name : 0}
              </h1>
              <h3
                className={
                  blockID ? `${styles.hotel_nightrates}` : `${styles.hide}`
                }
              >
              ...
              <button
                className={
                  blockID ? `${styles.reserve_button}` : `${styles.hide}`
                }
                onClick={handleShow}
              >
                Reserve
            ...
            <p
              className={
                blockID ? `${styles.hotel_photocount}` : `${styles.hide}`
              }
            >
              {" "}
              {`${currentSlide + 1}/${photoGallery.length}`}
            </p>
          </>
        ) : (
